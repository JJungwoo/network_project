...
int create_conn_st(int port)
{
	...
	memset(&gpconn.recvaddr, 0, sizeof(gpconn.recvaddr));
	...
	gpconn.recv_addr_len = sizeof(gpconn.sendaddr);
...
int main(int argc, char **argv)
{
	...
	if(ret == FAIL)
	{
		close_socket(gpconn.recv_sockfd);
		ret = create_socket();
		if(ret == FAIL) 
		{
			printf("create socker failed (ret:%d) \n", ret);
		...
		while(1)
		{
			...
			gpconn.send_sockfd = accept(gpconn.recv_sockfd, 
				(struct sockaddr*)&gpconn.recvaddr, (socklen_t *)&gpconn.recv_addr_len);

			if(gpconn.send_sockfd == FAIL)
			{
				printf("accept failed (ret:%d)(errno:%d) \n", gpconn.send_sockfd, errno);
				return FAIL;
			}
